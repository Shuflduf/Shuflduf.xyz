# My Personal Site
![88x31](/assets/88x31.gif)

# Features
- TWO WORLDS, LIGHT AND DARK
  - (There are two ways to view this website)
- **Switch between the Light and Dark worlds by finding the portal buttons**
  - In the Light world, this is accessed by pulling the sidebar out with the mouse
  - In the Dark world, its accessed by pulling down the "Personal" header in the navigation links
- [Ukiyo](https://github.com/technerium/obsidian-ukiyo) theme for Light world
- Indie Web styles for the Dark world (from [Neocities](https://neocities.org/))
- Full mobile support (except for some games)

## Games
### [Tetris](https://www.shuflduf.xyz/games/tetris.html)
*Also known as tetr.shuf*

The latest of [my many Tetris clones](https://www.shuflduf.xyz/projects/tetr-lang.html), integrated perfectly into both sides of the website.


### [CatGPT](https://www.shuflduf.xyz/games/catgpt.html)
The most useful AI (actual idiot) ever. Featuring [my cat](https://www.shuflduf.xyz/cat.html).


### [Rope Physics](https://www.shuflduf.xyz/games/rope.html)
It's literally just a single rope but it's really really fun to just throw around and stuff.


### [Rain Saber](https://www.shuflduf.xyz/games/rain-saber.html)
A combination of osu! and Fruit Ninja as a rhythm game. It's very unfinished, but if two people get to the end of what I mapped I'll finish it.

Song is [unbound](https://www.youtube.com/watch?v=lMCy1eDiFDw) by [fireentity](https://www.youtube.com/@fireentity_music)!


## Easter Eggs
There are 5 easter eggs going from easy to hard. If you get stuck trying to find one, you can consult [The Guide](https://www.shuflduf.xyz/).

If you manage to find all 5, send me proof. I doubt you will find all of them though.

![egg](/assets/egg.png)
